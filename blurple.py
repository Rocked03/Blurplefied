...
async def blurple(ctx, arg1=None):
    ...
    await ctx.send(
        f"{ctx.message.author.mention}, starting blurple image analysis (Please note that this may take a while)")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            if not is_gif:
                embed.set_image(url="attachment://image.png")
                embed.set_footer(
                    text=f"Please note - This blurplefier is automated and therefore may not always give you the best result. | Content requested by {ctx.author}")
            else:
                embed.set_image(url="attachment://image.gif")
                embed.set_footer(
                    text=f"Please note - This blurplefier is automated and therefore may not always give you the best result. Disclaimer: This image is a gif, and the quality does not always turn out great. HOWEVER, the gif is quite often not as grainy as it appears in the preview | Content requested by {ctx.author}")
            embed.set_thumbnail(url=picture)
            await ctx.send(embed=embed, file=image)
        except Exception:
            await ctx.send(
                f"{ctx.author.display.name}, whoops! It looks like this gif is too big to upload. If you want, you can give it another go, except with a smaller version of the image. Sorry about that!")
...
async def blurplefygif(ctx, arg1=None):
    ...
    await ctx.send(
        f'{ctx.message.author.display_name}, image fetched, analysing image (This process can sometimes take a while depending on the size of the image) ({end - start:.2f}s)')
    start = time.time()
